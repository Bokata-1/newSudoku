 ...
 * @author Boyanvb1
 */
public class SoutRand {
    public static void main(String[] args) throws FileNotFoundException {
        
        JFrame frame = new JFrame("Sudoku");
        RandomN obekt = new RandomN();
       SudokuGenerator h = new SudokuGenerator();
        frame.setSize(500, 500);
        frame.setLayout(new BorderLayout());
        JPanel panel = new JPanel();
        panel.setLayout(new GridLayout(9, 9));
        int masivIndex[][] = obekt.Random();
        int masivCh [][] = h.printBoard();
        String masivS[][] = new String[9][9];
        TextField[][] arr = new TextField[9][9];
        for (int i = 0; i < 9; i++) {
            for (int k = 0; k < 9; k++){
            masivS[i][k] = Integer.toString(masivCh[i][k]);      
            }
        }
        for (int i = 0; i < 9; i++) {
            for (int k = 0; k < 9; k++) {
        ...
        for (int i = 0; i < 1; i++) {
            for (int k = 0; k < 22; k++) {
                arr[masivIndex[i][k]][masivIndex[i + 1][k]].setText(masivS[masivIndex[i][k]][masivIndex[i + 1][k]]);
                arr[masivIndex[i][k]][masivIndex[i + 1][k]].setEditable(false);
            }
        }
